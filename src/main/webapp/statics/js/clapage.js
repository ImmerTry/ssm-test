...
function showPage(pageInfo) {
    ...
    var claList = pageInfo.list;//获取当前list信息
    $("#claList").html("");//清空表格中数据并重新填充数据

    for (var i = 0; i < claList.length; i++) {
        // var index = (pageNum - 1) * pageSize + i + 1;
        var tr = "<tr><td>" + '<input type="checkbox" value="${claList[i].classId}">' + "</td>"
            + '<td>' + claList[i].classId + '</td>'
            + '<td>' + claList[i].className + '</td>'
            + '<td>' + claList[i].viewKeyword + '</td>'
            + '<td>'
